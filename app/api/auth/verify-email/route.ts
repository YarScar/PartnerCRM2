...
export async function GET(req: NextRequest) {
  try {
    ...
    const ev = await (prisma as any).emailVerificationToken.findFirst({ where: { token, used: false, expiresAt: { gt: new Date() } } });
    if (!ev) return NextResponse.redirect(new URL('/settings?verified=error', req.url));

    // update user and mark token used
    await prisma.user.update({ where: { id: ev.userId }, data: { email: ev.email, email_verified: true, pending_email: null } as any });
    await (prisma as any).emailVerificationToken.update({ where: { id: ev.id }, data: { used: true } });
